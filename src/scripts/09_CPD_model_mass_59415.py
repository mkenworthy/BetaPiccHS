...
def plot_disktaumass2(tau,tau2, i_range, phi_range, mean_a, mean_a2, f_hill, f_hill2):
    ...
    print(np.max(dmass.value), np.min(dmass.value))

    r_disk2 = r_hill * f_hill2 # radius of disk
    tau_face_on2 = tau2 * (np.sin(i_range*np.pi/180.))[:,np.newaxis]
    dmass2 = disk_mass(r_disk, np.ma.filled(tau_face_on2, 0.0), mean_a = mean_a2)
    print(np.max(dmass2.value), np.min(dmass2.value))

    im1 = ax1.imshow(tau_face_on, 
                     extent=(phi_min-dphi,phi_max+dphi,i_min-di,i_max+di), 
                     vmin=0, 
                     vmax = v_max_upper_tau, 
                     cmap='viridis', 
                     origin='lower')
    im3 = ax3.imshow(dmass.value,    
                     extent=(phi_min-dphi,phi_max+dphi,i_min-di,i_max+di), 
                     vmin=0, 
                     vmax=v_max_mass, 
                     origin='lower',
    ...
    im2 = ax2.imshow(tau_face_on2, extent=(phi_min-dphi,phi_max+dphi,i_min-di,i_max+di), vmin=0, vmax = v_max_upper_tau, cmap='viridis', origin='lower')
    im4 = ax4.imshow(dmass2.value,    extent=(phi_min-dphi,phi_max+dphi,i_min-di,i_max+di), vmin=0, vmax=v_max_mass, origin='lower',cmap='viridis')


    print('For disk hill radius {:.2f} we have an upper mass of {:.2e}'.format(f_hill, np.max(dmass.value)))
    print('For disk hill radius {:.2f} we have an upper mass of {:.2e}'.format(f_hill2, np.max(dmass2.value)))
    ...
    ax1.set_title(r'Upper limit on $\tau$'+ '\n'+ r'corrected for inclination', fontsize=18)
    ax2.set_title(r'Upper limit on $\tau$'+ '\n'+ r'corrected for inclination', fontsize=18)
    ax3.set_title(r'Upper mass [g] of CPD' + '\n'+ r'assuming $D_{min}$'+r'={:.1f}'.format(mean_a.value)+r' $\mu m$', fontsize=18)
    ax4.set_title(r'Upper mass [g] of CPD' + '\n'+ r'assuming $D_{min}$'+r'={:.1f}'.format(mean_a2.value)+r' $\mu m$', fontsize=18)
    ...
    plt.draw()
    plotout = (paths.figures /r'{:.0f}_diskfit_mass.pdf'.format(t_mid.value))
    plt.savefig(plotout)

def return_upper_tau(bring_disk, brite_disk, astep_disk, f_hill):
    ...
    print('Mass estimate from Perez+ 2019: {:.1e}'.format((bp.M_c * 5e-8).to(u.g)))
    return upper_tau_max, mean_a
...
upper_tau_max_03, mean_a_03 = return_upper_tau(bring_disk_03, brite_disk_03, astep_disk_03, f_hill=0.3)
upper_tau_max_06, mean_a_06 = return_upper_tau(bring_disk_06, brite_disk_06, astep_disk_06, f_hill=0.6)

plot_disktaumass2(upper_tau_max_03, upper_tau_max_06, i_range, phi_range,  mean_a_03,  mean_a_06, f_hill=0.3, f_hill2=0.6)